...
Loading:SetMessage("Inicializando XOCU...")
Loading:SetDescription("Carregando script...")
Loading.Sidebar:AddLabel("Versão: 3.0.0")
Loading.Sidebar:AddLabel("Usuário: " .. game.Players.LocalPlayer.Name)
Loading:SetCurrentStep(1)

task.wait(0.5)
Loading:SetCurrentStep(2)
Loading:SetDescription("Conectando ao jogo...")
...
-- ============================================================================
-- CRIAÇÃO DA JANELA PRINCIPAL (OBSIDIAN)
-- ============================================================================
Loading:SetCurrentStep(3)
Loading:SetDescription("Criando interface...")

local Window = Library:CreateWindow({
    Title = "XOCU",
    Footer = "XOCU v3.0 | Obsidian",
...
-- ============================================================================
local Tabs = {
    Defense = Window:CreateTab("Defense", "7461510456"),
    Target = Window:CreateTab("Target", "107058246184363"),
    Grab = Window:CreateTab("Grab", "85607241723723"),
    Player = Window:CreateTab("Player", "124871982298256"),
    Misc = Window:CreateTab("Misc", "114167292947807"),
    Keybinds = Window:CreateTab("Keybinds", "4814130203"),
    Visuals = Window:CreateTab("Visuals", "7733955511"),
}

Loading:SetCurrentStep(4)
Loading:SetDescription("Carregando addons...")
...
SaveManager:SetFolder("XOCU")
SaveManager:SetSubFolder("Configs")
SaveManager:BuildConfigSection(Tabs.Misc, "settings")

-- ============================================================================
-- CONFIGURAÇÃO DO THEMEMANAGER
-- ============================================================================
ThemeManager:SetLibrary(Library)
ThemeManager:SetFolder("XOCU")
ThemeManager:ApplyToTab(Tabs.Misc, "palette")
...
-- ============================================================================
-- FUNÇÃO DE LOADING COMPLETO
-- ============================================================================
function sendHubLoadedMessage()
    local message = "XOCU v3.0 | Obsidian loaded."
    local sent = false
    pcall(function()
        ...
        if v then
            _G.AutoResetCon = game:GetService("ReplicatedStorage").GameCorrectionEvents.GameCorrectionsNotify.OnClientEvent:Connect(function(r)
                if r == "Flying" then
                    ...
                    if hum then
                        Library:Notify({ Title = "System", Description = "Resetting to prevent Ban", Time = 4 })
                        char:BreakJoints() 
                        hum.Health = 0
                    end
                end
...
DefenseGroup:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            task.spawn(function() PerformLegDeletion(Player.Character) end)
            ...
            if hum and hum.Health > 0 and (not char:FindFirstChild("Left Leg") or not char:FindFirstChild("Right Leg")) then
                Library:Notify({ Title = "System", Description = "Legs cannot be restored locally. Reset your character.", Time = 4 })
            end
        end
    end
})
        ...
        Callback = function(Value)
            ...
            if Value then
                ...
                local function gucci()
                    ...
                    while (ragdolled and ragdolled.Value) or BeingHeld.Value do task.wait() end
...
Player.CharacterAdded:Connect(function()
    if _G.ShurikenAntiKick then task.wait(1) end
end)
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            if not tt or not tt.Character or not tt.Character:FindFirstChild("HumanoidRootPart") then
                Library:Notify({ Title = "Error", Description = "Select a valid target!", Time = 3 })
                return
            end
            task.spawn(function()
...
do
    local function snapBlobTo(blob, targetCFrame)
        ...
        ap.Responsiveness = 300        ap.MaxForce = math.huge
    ...
    BlobGroup:CreateButton({
        ...
        Callback = function()
            ...
            if not blob then
                Library:Notify({ Title = "Error", Description = "Blobman not found!", Time = 3 })
                return
            end
            local players = {}
            ...
            if #players == 0 then
                Library:Notify({ Title = "Error", Description = "No players to kick!", Time = 3 })
                return
            end
            Library:Notify({ Title = "Test Bomba", Description = "Starting silent mass kick...", Time = 3 })
            ...
            local baseRadius = 40
            local radius = baseRadius + (#players * 5)
            ...
            Library:Notify({ Title = "Success", Description = "Test Bomba execution complete!", Time = 3 })
        end
    })
end

-- ============================================================================
-- RAGDOLL SPAM (PALLET HAMMER)
-- ============================================================================
do
    local SpamSetOwner = {
        AutoRagdoll = false,
...
TargetGroup:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            if not selectedKickPlayer then
                Library:Notify({ Title = "System", Description = "Select target first", Time = 3 })
                return
            end
...
do
    ...
    local function performKill()
        ...
        if not char:GetAttribute("SavingOriginalPos") then saveOriginalPos() end
...
do
    ...
    local function OwnershipKickFunction(targetName)
        ...
        local currentFPS = 60
        local fpsConn = R.RenderStepped:Connect(function(dt) currentFPS = 1 / dt end)
        local bodyPos = nil
        local bodyGyro = nil
                    ...
                    while bool.LoopGrabKick and RunService.RenderStepped:Wait() do 
                        ...
                        if (etc.Head and etc.Hum and etc.Root and etc.Hum.Health > 0) and (currentHRP.Position - etc.Root.Position).Magnitude <= 30 then  
                            ...
                            while (etc.Head and etc.Hum and etc.Hum.Health > 0) and task.wait(0.01) and bool.LoopGrabKick do 
                                ...
                                sno(etc.Head)
                                if CheckForPartOwner(etc.Head) then break end
                            end
            ...
            if Value then 
                task.spawn(function()
                    ...
                    if not etc.TargetPLR then 
                        Library:Notify({ Title = "System", Description = "Error: Target does not exist!", Time = 3 })
                        return 
                    end 
...
do
    ...
    local function spawntoy(toy, cf)
        ...
        local toyadded = inv.ChildAdded:Connect(function(c)
            if c.Name == toy then t = c end
        end)
        task.spawn(function()
            pcall(function() SpawnToy:InvokeServer(toy, cf, Vector3.new(0,0,0)) end)
        end)
        local time = tick() + 2
    ...
    local function tp(obj1, obj2)
        ...
        obj1.CFrame = CFrame.new(obj2.Position + obj2.Velocity * (dataPing / 1000) * 5)
    end
            ...
            if Value then
                if not selectedKickPlayer then
                    if Library and Library.Notify then
                        Library:Notify({ Title = "Error", Description = "Select a target first!", Time = 3 })
                    end
                    applymethod = false
                    return
                end
                spamGrabKickTask = task.spawn(function()
                    ...
                    while applymethod and RunService.RenderStepped:Wait() do
                        ...
                        local inv = workspace:FindFirstChild(plr.Name.."SpawnedInToys")
                        local tt = selectedKickPlayer
                        if not tt or not tt.Character or not tt.Character:FindFirstChild("HumanoidRootPart") or not HRP or not inv then break end
                        local Sets = {
                            Char = tt.Character,
    ...
    Callback = function(on)
        ...
        task.spawn(function()
            ...
            if not target or not target.Parent then
                ...
                Library:Notify({ Title = "Oats Kick", Description = "No target selected.", Time = 3 })
                return
            end
            ...
            if target and target.Character then
                ...
                if tH then
                    ...
                    pcall(function() ReplicatedStorage.GrabEvents.DestroyGrabLine:FireServer(tH) end)
                end
            end
            pcall(function()
            ...
            while LGK_OAT_kickLoopEnabled and RunService.Heartbeat:Wait() do
                ...
                if dist > 30 then
                    ...
                    while (tick() - grabStartTime) < 0.3 do
                        task.wait(0.05)
                        if type(LGK_OAT_sno) == "function" then LGK_OAT_sno(tHRP) end
                        pcall(function() ReplicatedStorage.GrabEvents.DestroyGrabLine:FireServer(tHRP) end)
                        local align = tHRP:FindFirstChild("KickAlign")
...
TargetGroup:CreateToggle({
    ...
    Callback = function(Value)
        ...
        _G.RagdollSnowballActive = Value
                ...
                if target and target.Character then
                    ...
                    if torso then
                        pcall(function()
                            ...
                            task.spawn(function() SpawnRemote:InvokeServer("BallSnowball", torso.CFrame * CFrame.new(offset), Vector3.zero) end)
                        end)
...
-- ============================================================================
-- USER INPUT SERVICE - SIT ON BLOBMAN
-- ============================================================================
...
PlayerView:CreateToggle({
    ...
    Callback = function(v)
        ...
        for i,vv in pairs(workspace.Map.AlwaysHereTweenedObjects.Ocean.Object.ObjectModel:GetChildren()) do
            if vv.Name == "Ocean" then vv.CanCollide = v end
        end
    end
})
        ...
        if Value then
            ...
            for _, plr in pairs(PS:GetPlayers()) do
                if plr.Character then
                    ...
                    if hrp and hrp:FindFirstChild("NameESP") then hrp.NameESP:Destroy() end
                end
            end
        end
    end
        ...
        for _, v in pairs(Workspace:GetDescendants()) do
            if v:IsA("BasePart") then
                if not oldProperties[v] then
                    oldProperties[v] = { Material = v.Material, Reflectance = v.Reflectance, CastShadow = v.CastShadow }
                end
                ...
                for _, part in pairs(plr.Character:GetDescendants()) do
                    if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
                        if not oldProperties[part] then
                            oldProperties[part] = { Material = part.Material, Reflectance = part.Reflectance, CastShadow = part.CastShadow }
                        end
...
do
    ...
    MiscGroup:CreateToggle({
        ...
        Callback = function(Value)
            ...
            if Value then
                startPacketDetector()
                Library:Notify({ Title = "Enabled", Description = "Packet detector active", Time = 4 })
            else
                stopPacketDetector()
                Library:Notify({ Title = "Disabled", Description = "Packet detector off", Time = 4 })
            end
        end
    })
end
...
do
    ...
    local function hamburgerTeleport()
        ...
        if DestroyToy then DestroyToy:FireServer(FoodHamburger) end
            ...
            if Value then
                task.spawn(function()
                    ...
                    while _G.MonsterLagEnabled and CreateLine do
                        local spawnLocation = WS:FindFirstChild("SpawnLocation") or WS:FindFirstChild("Spawn") or (LP.Character and LP.Character:FindFirstChild("HumanoidRootPart"))
                ...
                Library:Notify({ Title = "Monster Lag", Description = "Enabled", Time = 2 })
            else
                Library:Notify({ Title = "Monster Lag", Description = "Disabled", Time = 2 })
            end
        end
    })
end
...
MiscGroup:CreateSlider({
    ...
    Max = 5000,
    Rounding = 0,
    Callback = function(v) PacketSpamAmount = v end
})
        ...
        if Value then
            task.spawn(function()
                ...
                while _G.PacketLagActive do
                    pcall(function() GrabEvent:FireServer(string.rep("Genesis Was Here", PacketSpamAmount)) end)
                    task.wait()
                end
            end)
        end
...
do
    ...
    function notifyKick(displayName, username)
        Library:Notify({ Title = "XOCU", Description = displayName .. " (" .. username .. ") has been kicked", Time = 6 })
    end
            ...
            task.spawn(function()
                ...
                while _G.En do
                    ...
                    if #Coconuts < (Length + 2) then
                        task.spawn(function() SpawnToy:InvokeServer("FoodCoconut", Root.CFrame * CFrame.new(-5, 0, 10), Vector3.zero) end)
                    end
                    for i, Coco in ipairs(Coconuts) do
                        ...
                        if Part and HoldPart and Rigid then
                            ...
                            if Rigid.Attachment1 then DestroyToy:FireServer(Coco) end
                            for _, part in pairs(Coco:GetChildren()) do
                                if part:IsA("BasePart") then
                                    part.CanCollide = false
                                    part.CanQuery = false
                                    if part.Transparency ~= 1 then part.Transparency = 0
                                end
                            end
                        end
                    end
            ...
            task.spawn(function()
                ...
                heartConnection = RunService.Heartbeat:Connect(function(dt)
                    ...
                    if not part or not part.Parent then
                        if heartToy and heartToy.Parent then pcall(function() heartToy:Destroy() end) end
                        heartToy = nil
                        task.spawn(function()
                            ...
                            pcall(function() RS.MenuToys.SpawnToyRemoteFunction:InvokeServer("FireworkSparkler", hrp2.CFrame * CFrame.new(0, 50, 0), Vector3.zero) end)
...
do
    ...
    local function startDogPose()
        ...
        if not target then 
            Library:Notify({ Title = "System", Description = "No target found under mouse!", Time = 3 })
            return 
        end
        ...
        if not (Torso and Head and Hum) then 
            SpamChar = nil
            Library:Notify({ Title = "System", Description = "Invalid character target!", Time = 3 })
            return 
        end
        ...
        Library:Notify({ Title = "Dog Pose", Description = "Locked onto " .. SpamChar.Name, Time = 3 })
        LoopGrabDogConn = RunService.Heartbeat:Connect(function()
            if not loopGrabDogActive or not hrp or not SpamChar or not SpamChar.Parent then stopDogPose() return end
            Torso = SpamChar:FindFirstChild("Torso") or SpamChar:FindFirstChild("UpperTorso")
            Head = SpamChar:FindFirstChild("Head")
            if not Torso or not Head then stopDogPose() return end
    ...
    -- Refined Anatomy Version
    do
        local function stopDogPoseRefined()
            loopGrabDogActive = false
            if LoopGrabDogConn then LoopGrabDogConn:Disconnect(); LoopGrabDogConn = nil end
        ...
        local function startDogPoseRefined()
            local Mouse = LocalPlayer:GetMouse()
            local target = Mouse.Target
            ...
            LoopGrabDogConn = RunService.Heartbeat:Connect(function()
                if not loopGrabDogActive or not hrp or not SpamChar:FindFirstChild("Torso") then stopDogPoseRefined() return end
                local Torso = SpamChar.Torso
                local Head = SpamChar:FindFirstChild("Head")
                if snoRemote then pcall(function() snoRemote:FireServer(Head, Head.CFrame) end) end
                Torso.CFrame = hrp.CFrame * CFrame.new(0, -1.2, -2.5) * CFrame.Angles(math.rad(-90), 0, 0)
                if Head then Head.CFrame = Torso.CFrame * CFrame.new(0, 1.3, -0.2) * CFrame.Angles(math.rad(45), 0, 0) end
                local LArm = SpamChar:FindFirstChild("Left Arm")
                local RArm = SpamChar:FindFirstChild("Right Arm")
                local LLeg = SpamChar:FindFirstChild("Left Leg")
                local RLeg = SpamChar:FindFirstChild("Right Leg")
    ...
    KeybindsGroup:CreateKeybind({
        ...
        Callback = function()
            ...
            if newState then
                ...
                stopDogPose()
                Library:Notify({ Title = "Dog Pose", Description = "Deactivated", Time = 3 })
            end
        end
    })
end
...
do
    ...
    function FGM.ToggleAutoRagdoll(enabled)
        ...
        if not enabled then
            if FGM.State.RagdollPallet then pcall(function() FGM.DestroyToy:FireServer(FGM.State.RagdollPallet) end) end
        ...
        task.spawn(function()
            ...
            FGM.State.AutoRagdollConnection = FGM.RunService.Heartbeat:Connect(function()
                ...
                local targets = {}
                if FGM.State.FigureGrabEnabled and FGM.State.TargetCharacter then table.insert(targets, FGM.State.TargetCharacter) end
    ...
    function FGM.ToggleFigureGrab()
        if not FGM.State.FigureGrabEnabled then
            ...
            if not MouseTarget then
                Library:Notify({Title = "Error", Description = "Aim at a player first", Time = 3})
                return
            end
            FGM.State.TargetCharacter = MouseTarget.Parent
            local MyCharacter = FGM.GetCharacter(FGM.LocalPlayer)
            if not FGM.State.TargetCharacter or not MyCharacter then
                Library:Notify({Title = "Error", Description = "Invalid target", Time = 3})
                return
            end
            local BodyParts = {"Head", "Left Arm", "Right Arm", "Left Leg", "Right Leg"}
            local TargetTorso = FGM.State.TargetCharacter:FindFirstChild("Torso")
            if not TargetTorso then
                Library:Notify({Title = "Error", Description = "Torso not found", Time = 3})
                return
            end
            ...
            if FGM.State.FigureGrabConnection then FGM.State.FigureGrabConnection:Disconnect() end
            ...
            if FGM.State.AutoRagdollToggle then FGM.ToggleAutoRagdoll(true) end
            Library:Notify({Title = "Figure Grab", Description = "Figure Grab Activated", Time = 3})
        else
            FGM.State.FigureGrabEnabled = false
            ...
            Library:Notify({Title = "Figure Grab", Description = "Figure Grab Deactivated", Time = 3})
        end
    end
    ...
    function FGM.ResetPose()
        for section, values in pairs(FGM.Configuration) do
            if typeof(values) == "table" then
                for axis, _ in pairs(values) do values[axis] = 0 end
            end
        end
    end
    ...
    FigureMain:CreateKeybind({
        ...
        Default = "V",
        Callback = function() FGM.ToggleFigureGrab() end,
    })
    ...
    FigureMain:CreateToggle({
        ...
        Callback = function(Value)
            ...
            if FGM.State.FigureGrabEnabled then FGM.ToggleAutoRagdoll(Value) end
        end,
    })
    ...
    FigureMain:CreateToggle({
        ...
        Callback = function(Value)
            ...
            if Value then
                Library:Notify({Title = "Animation Copy", Description = "Now copying your animations!", Time = 3})
            else
                Library:Notify({Title = "Animation Copy", Description = "Manual control restored", Time = 3})
            end
        end,
    })
    ...
    Callback = function(Value)
        ...
        if Value then
            task.spawn(function()
                ...
                function invis_touch(part, cf) SetNetOwner:FireServer(part, cf) end
...
AnimationsGroup:CreateDropdown({
    Name = "Animation",
    Flag = "Animation",
    Items = { "Crazy", "Insane", "Collapse", "Zombie" },
    Default = "Crazy",
    Callback = function(v)
        selectedAnimName = v
...
do
    ...
    FX_defs["Orbit Rings"] = function()
        ...
        for i = 1, count do
            ...
            table.insert(ringData,{ part=(ring), offset=(i-1)*(math.pi*2/count), tilt=(i-1)*(math.pi/count) })
        end
        local t=0
...
local function applyWhiteHoleVisuals(model)
    ...
    if _G.WhiteHoleEnabled then
        ...
        if beam then beam.Color = ColorSequence.new(Color3.fromRGB(255, 255, 255)) end
        ...
        local beam = hole:FindFirstChild("Attachment") and hole.Attachment:FindFirstChild("Beam")
        if beam then beam.Color = ColorSequence.new(Color3.fromRGB(170, 0, 255)) end
        local gui = hole:FindFirstChild("BillboardGui")
        if gui then
...
BlackHoleSettings:CreateToggle({
    ...
    Callback = function(enabled)
        ...
        if current then applyWhiteHoleVisuals(current) end
...
BlackHoleSettings:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            local current = workspace:FindFirstChild("BlackHoleKick")
            if current then applyRealisticBH(current) end
...
BlackHoleSettings:CreateSlider({
    ...
    Callback = function(v)
        ...
        if model then
            local hole = model:FindFirstChild("Hole")
            local beam = hole and hole:FindFirstChild("Attachment") and hole.Attachment:FindFirstChild("Beam")
            if beam then beam.Transparency = NumberSequence.new(v / 100) end
        end
    end
})
